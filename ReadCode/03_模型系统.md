...
from_pretrained()
  │
  ├─ 1. 参数解析与兼容性处理
  │     ├─ 提取 kwargs（dtype, device_map, quantization_config 等）
  │     ├─ torch_dtype → dtype 兼容
  │     └─ TP 初始化（tp_plan/tp_size → device_mesh）
  │
  ├─ 2. 加载配置
  ...
  │
  ├─ 7. 在上下文管理器中实例化模型
  │     ├─ model_init_context = cls.get_init_context(dtype, ...)
  │     │   ├─ local_torch_dtype(dtype)     # 设置默认 dtype
  │     │   ├─ no_tie_weights()              # 延迟权重绑定
  │     │   ├─ apply_patches()               # 应用 monkey patch
  │     │   ├─ torch.device("meta")          # meta 设备初始化
  ...
  │
  ├─ 10. 张量并行分发
  │      └─ model = distribute_model(model, tp_plan, ...)
  │
  ├─ 11. 准备设备映射
  │      └─ device_map = _get_device_map(model, device_map, max_memory, ...)
  │
  ├─ 12. 加载权重
  │      ├─ LoadStateDictConfig(...)  # 打包加载配置
  │      ├─ _load_pretrained_model(model, state_dict, checkpoint_files, load_config)
  │      │   ├─ safetensors: 延迟加载（get_slice 不立即物化）
  │      │   ├─ .bin: 全量加载
  │      │   └─ convert_and_load_state_dict_in_model(model, state_dict, load_config, ...)
  │      └─ _finalize_model_loading(model, load_config, loading_info)
