...
graph TB
    subgraph "pytest 配置层"
        A[conftest.py<br/>全局配置/钩子]
        B[pytest_configure<br/>注册标记]
    end
    
    subgraph "Mixin 测试层"
        C[ModelTesterMixin<br/>模型通用测试]
        D[ConfigTester<br/>配置通用测试]
        E[TokenizerTesterMixin<br/>分词器通用测试]
        F[PipelineTestMixin<br/>Pipeline 测试]
        G[TrainingTestMixin<br/>训练测试]
        H[TensorParallelTestMixin<br/>张量并行测试]
    end
    
    subgraph "模型特定测试层"
        I[test_modeling_{name}.py]
        J[test_configuration_{name}.py]
        K[test_tokenization_{name}.py]
    end
    
    subgraph "专项测试层"
        L[generation/<br/>生成测试]
        M[trainer/<br/>训练器测试]
        N[pipelines/<br/>Pipeline 测试]
        O[cli/<br/>CLI 测试]
    end
