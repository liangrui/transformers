...
用户访问 transformers.BertModel
  → _LazyModule.__getattr__("BertModel")
    → 检查 _objects（额外对象缓存）
    → 检查 _object_missing_backend（缺少后端的对象）
      → 若缺少后端：创建 Placeholder（DummyObject 元类），访问任何方法时抛 ImportError
      → 特殊处理：ImageProcessor 的 PIL 回退、TokenizerFast 的回退
    → 检查 _class_to_module（对象名 → 模块路径映射）
      → 调用 _get_module() 真正导入模块
      → getattr(module, name) 获取实际对象
    → 检查 _modules（子模块集合）
    → 最终 setattr(self, name, value) 缓存结果
```
...
```
1. __init__.py 执行
2. from . import dependency_versions_check
   → 检查核心依赖版本
3. from .utils import _LazyModule, is_*_available, ...
   → 建立后端检测函数
4. 构建 _import_structure 字典
   → 根据后端可用性动态填充
5. define_import_structure(models/)
   → 自动扫描 models/ 目录
6. sys.modules[__name__] = _LazyModule(...)
   → 替换模块为懒加载实例
7. 用户访问 transformers.BertModel
   → _LazyModule.__getattr__("BertModel")
   → _get_module("models.bert.modeling_bert")
   → importlib.import_module(".models.bert.modeling_bert", "transformers")
   → 返回 BertModel 类并缓存
```
...
```
1. 用户访问 transformers.BlipForConditionalGeneration
2. _LazyModule.__getattr__("BlipForConditionalGeneration")
3. 发现 "BlipForConditionalGeneration" 在 _object_missing_backend 中
4. 创建 Placeholder(metaclass=DummyObject, _backends=["torch"])
5. 返回 Placeholder 类
6. 用户尝试实例化 Placeholder(...)
7. DummyObject.__getattribute__ 拦截
8. requires_backends(Placeholder, ["torch"])
9. 抛出 ImportError:
   "BlipForConditionalGeneration requires the PyTorch library but it was not
    found in your environment. Check out the instructions on the installation
...
```
1. PreTrainedModel.from_pretrained("bert-base-uncased")
2. → cached_file("bert-base-uncased", "config.json")
3. → cached_files(["config.json"])
4.   → 离线模式检查
5.   → 本地目录检查（否）
6.   → 缓存命中检查（使用 commit_hash）
7.   → hf_hub_download(...) 或 snapshot_download(...)
8.   → 异常处理（分级）
9.   → 返回缓存文件路径
```
