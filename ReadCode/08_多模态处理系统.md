...
graph TB
    subgraph "统一入口层"
        A[ProcessorMixin<br/>PushToHubMixin]
        B[__call__ 多模态统一入口]
    end
    
    subgraph "子处理器层"
        C[Tokenizer<br/>文本处理]
        D[ImageProcessor<br/>图像处理]
        E[VideoProcessor<br/>视频处理]
        F[FeatureExtractor<br/>特征提取]
    end
    
    subgraph "后端层"
        G[PilBackend<br/>CPU后端]
        H[TorchvisionBackend<br/>GPU后端]
    end
    
    subgraph "变换层"
        I[Resize<br/>Rescale<br/>Normalize<br/>Pad]
    end
    
    subgraph "工具层"
        J[image_utils<br/>video_utils<br/>audio_utils<br/>vision_utils]
    end
