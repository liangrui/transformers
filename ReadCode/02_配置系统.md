...
save_pretrained(save_directory)
  ├── 检查是否有生成参数混入 → 有则报错
  ├── 创建目录
  ├── 如果有 _auto_class → 保存自定义对象文件
  ├── 调用 validate() 进行严格验证
  └── to_json_file(config.json, use_diff=True)
        └── to_json_string(use_diff=True)
              └── to_diff_dict()
                    ├── to_dict() → 深拷贝 __dict__
                    │     ├── 处理嵌套 PreTrainedConfig → 递归 to_dict()
                    │     ├── tuple → list 转换
                    │     ├── 添加 transformers_version
                    │     ├── 处理 quantization_config
                    │     └── dict_dtype_to_str() → torch.float32 → "float32"
                    ├── 与默认 PreTrainedConfig() 对比
                    ├── 与当前类默认值对比
                    ├── 递归处理嵌套配置的 diff
                    ├── _remove_keys_not_serialized() → 移除内部字段
                    └── _encode_special_floats() → Infinity/NaN 编码
```
...
from_pretrained(pretrained_model_name_or_path, **kwargs)
  ├── get_config_dict() → 获取原始字典
  │     └── _get_config_dict()
  │           ├── cached_file() → 从 Hub/本地/缓存解析文件
  │           ├── _dict_from_json_file() → JSON 解析
  │           ├── _decode_special_floats() → 还原 Infinity/NaN
  │           └── 处理 configuration_files 版本选择
  ├── base_config_key 提取嵌套配置
  ├── model_type 匹配检查
  └── from_dict(config_dict, **kwargs)
        ├── 合并 kwargs 中的有效字段到 config_dict
        ├── cls(**config_dict) → 实例化
        ├── 处理嵌套 PreTrainedConfig 的 kwargs 覆盖
        └── 返回 config（或 config + unused_kwargs）
```
...
```
PushToHubMixin
    ├── PreTrainedConfig (同时继承 RotaryEmbeddingConfigMixin)
    │     ├── BertConfig
    │     ├── LlamaConfig
    │     ├── CLIPConfig (包含子配置: text_config, vision_config)
    │     └── ... 所有模型配置
    └── GenerationConfig

RotaryEmbeddingConfigMixin
    └── PreTrainedConfig (多重继承)
...
DistributedConfig (独立 dataclass)
```
...
AutoConfig.from_pretrained("model_name")
  │
  ├── 1. 解析 model_type → 查找 CONFIG_MAPPING
  │
  ├── 2. 调用 SpecificConfig.from_pretrained()
  │     ├── get_config_dict() → 下载/读取 config.json
  │     │     ├── 支持 GGUF 格式
  │     │     ├── 支持 timm 模型
  │     │     └── 支持 configuration_files 版本选择
  │     │
  │     └── from_dict(config_dict)
  │           ├── 合并 kwargs 覆盖
  │           ├── cls(**config_dict) → dataclass __init__
  │           │     └── __post_init__(**kwargs)
  │           │           ├── torch_dtype → dtype 兼容
  │           │           ├── rope_scaling → rope_parameters 标准化
  │           │           ├── 弹出生成参数
  │           │           ├── 设置 attn_implementation（递归子配置）
  │           │           └── 设置额外 kwargs 属性
  │           │
  │           └── kwargs 覆盖已有属性
  │
